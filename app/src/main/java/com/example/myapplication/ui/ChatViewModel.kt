...
class ChatViewModel(
    ...
    fun send() {
        ...
        viewModelScope.launch {
            // Keep current messages if we are in Ready state
            ...
            when (repository.sendMessage(myName, draft)) {
                ...
                is AppResult.Failure -> {
                    // Only show full error screen if we don't have messages
                    if (previousState !is ChatUiState.Ready) {
                        uiState = ChatUiState.Error("Could not send. Check your connection.")
                    }
                    // In a real app, we'd use a Snackbar here
                }
            }
        }
    }
